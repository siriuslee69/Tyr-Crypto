...
- Reusable primitives and SIMD backends; protocol constructions remain in their
  owning repositories.
...
- Independent-stream XChaCha20 batches use eight AVX2 lanes or four SSE2/NEON
  lanes with exact scalar tails and block-counter validation.
- The former ChunkyCrypto construction moved to Bifrost as CHUNKYAEAD; Tyr now
  owns only its reusable AES, XChaCha20, Gimli, and hash primitives.
- Verification passed for scalar and AVX2/SSE2 batch equivalence, ARM64/NEON
  compile checks, `nimble check_core`, the 49-entry WebUI catalog contract, and
  all 24 desktop test groups.
...
- `nim r tools\\ensure_env.nim -- --submodules` (fails in this sandbox because the default local nimcache path is not writable)
...
- Regular desktop suite:
  ...
  - the enclosing `nimble test` command returned exit 1 only after tests completed because Nimble could not save its local metadata in the earlier restricted sandbox.
- Android:
  - `powershell -File tools/build_android_harness.ps1 -HarnessTarget asymmetric_full -Release` (pass)
  - Infinix X6871: asymmetric-full harness pass, `exit=0`.
  - Motorola Edge 50 Fusion: asymmetric-full harness pass, `exit=0`.
  - Moto G56 5G: asymmetric-full harness pass, `exit=0`.
