...
suite "signatures wrapper":
  ...
  test "sphincs availability is provided by custom crypto":
    check signatureAvailable(saSPHINCSPlusShake128fSimple)

  when defined(release):
    test "sphincs wrapper uses deterministic pure Nim key generation":
      var
        seed = newSeq[uint8](48)
        i: int = 0
      while i < seed.len:
        seed[i] = uint8(i + 1)
        i = i + 1
      let kp0 = signatureKeypair(saSPHINCSPlusShake128fSimple, seed)
      let kp1 = signatureKeypair(saSPHINCSPlusShake128fSimple, seed)
      let msg = @[6'u8, 2'u8, 6'u8]
      let sig = signMessage(saSPHINCSPlusShake128fSimple, msg, kp0.secretKey)
      check kp0.publicKey == kp1.publicKey
      check kp0.secretKey == kp1.secretKey
      check verifyMessage(saSPHINCSPlusShake128fSimple, msg, sig,
        kp0.publicKey)
