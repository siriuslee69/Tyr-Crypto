...
proc parseConfig(): RunConfig =
  var
    i: int = 1
    arg: string = ""
  result.serial = getEnv("ANDROID_SERIAL", "")
...
when isMainModule:
  var
    ...
    repoRoot: string = repoRootFrom(currentSourcePath())
    apkPath: string = ""
    packageName: string = "org.tyrcrypto.harness"
    component: string = packageName & "/" & packageName & ".MainActivity"
    deadline: DateTime
    output: string = ""
  apkPath = joinPath(repoRoot, "evaluation", "tests", "android_harness", "app",
  ...
  if config.serial.len == 0:
    raise newException(ValueError,
      "Android serial is required through ANDROID_SERIAL or --serial")
