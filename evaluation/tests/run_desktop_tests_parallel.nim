...
proc buildGroups(): seq[TestGroup] =
  ...
  addGroup(result, "kdf_prng", ["test_kdf_prng_memory.nim"])
  ...
  addGroup(result, "classical", ["test_classical_asymmetric_boundaries.nim"])
  ...
  addGroup(result, "kem_material", ["test_kem_material_tyr.nim"])
...
proc resolveNimblePackagePath(name: string): string =
  var
    probe: tuple[output: string, exitCode: int]
    lines: seq[string] = @[]
    i: int = 0
    candidate: string = ""
  probe = execCmdEx("nimble path " & quoteShell(name))
  if probe.exitCode != 0:
    raise newException(OSError, "cannot resolve Nimble package " & name)
  ...
  while i < lines.len:
    ...
    i = i + 1
  if result.len == 0:
    raise newException(OSError, "Nimble returned no directory for " & name)
...
proc runGroup(g: TestGroup, root, logRoot: string, defines,
  ...
  while i < g.tests.len:
    ...
    outPath = joinPath(root, "build", "test_parallel_" & g.name & "_" &
      testBase & ExeExt)
    args = @["c", "--nimcache:" & cache, "--out:" & outPath]
    j = 0
    ...
    while j < defines.len:
      ...
      j = j + 1
    args.add("-r")
    args.add(joinPath("evaluation", "tests", g.tests[i]))
...
when isMainModule:
  ...
  if config.full:
    ...
    defines.add("-d:hasNimcrypto")
    defines.add("-d:hasLiboqs")
    defines.add("-d:hasLibsodium")
    defines.add("-d:hasOpenSSL3")
    dependencyPath = getEnv("TYR_NIMCRYPTO_PATH").strip()
    if dependencyPath.len == 0:
      dependencyPath = resolveNimblePackagePath("nimcrypto")
      putEnv("TYR_NIMCRYPTO_PATH", dependencyPath)
    flags.add("--path:" & dependencyPath)
