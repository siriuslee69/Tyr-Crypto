...
suite "otp custom crypto":
  ...
  test "weak secrets and nonstandard digit counts are rejected":
    expect ValueError:
      discard hotpRfc4226(toBytes("short"), 0'u64)
    expect ValueError:
      discard hotpRfc4226(toBytes("1234567890123456"), 0'u64, 4)
    expect ValueError:
      discard hotpRfc4226(toBytes("1234567890123456"), 0'u64, 9)
  ...
  when defined(sse2):
    test "simd sse2 matches scalar":
      let secret = toBytes("otp-secret-sse2-long")
      for algo in OtpAlgo:
  ...
  when defined(avx2):
    test "simd avx2 matches scalar":
      let secret = toBytes("otp-secret-avx2-long")
      for algo in OtpAlgo:
