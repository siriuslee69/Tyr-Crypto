...
suite "gimli sse":
  when defined(emscripten) or defined(tyrWasm):
    test "wasm scalar permutation is deterministic":
      var
        s0: Gimli_Block
        s1: Gimli_Block
        i: int = 0
  ...
  when defined(neon) or defined(arm64) or defined(aarch64):
    # The NEON path is the same generic GimliVec4 code as the SSE path; these
    # run it again on ARM with different vectors.
