...
when defined(hasLibsodium):
  ...
  proc algoKeyCount(a: CipherSuite): int =
    ## a: algorithm for which to compute key count.
    result = cryptoApi.keyCount(a)

  proc algoKeyBytes(a: CipherSuite): int =
    ## a: algorithm for which to compute key bytes.
    let c = algoKeyCount(a)
    result = c * 32

  proc algoNonceLen(a: CipherSuite): int =
    ## a: algorithm for which to compute nonce length.
    result = cryptoApi.nonceBytes(a)

  proc algoContext(a: CipherSuite): string =
    ...
    of csAes256Gcm:
      ...
      raise newException(ValueError, "cipher suite is not supported for pin-key derivation")
  ...
  proc splitSymmetricKeys(ms: seq[uint8], c: int): seq[seq[uint8]] =
    ## ms: raw key material.
    ...
    while i < c:
      offset = i * 32
      ks[i] = ms[offset ..< offset + 32]
      i = i + 1
    result = ks

  proc resolveNonce(a: CipherSuite, ns: openArray[uint8]): seq[uint8] =
  ...
  proc deriveSymmetricKeysFromBytes*(a: CipherSuite,
  ...
  proc deriveSymmetricKeysFromString*(a: CipherSuite,
  ...
  proc derivePasswordPinSecretFromBytesWithSalt*(ps, pins, ss: openArray[uint8],
    ...
    discard ml
    discard l
    raiseUnavailable("Argon2 password and PIN derivation", "hasLibsodium")
