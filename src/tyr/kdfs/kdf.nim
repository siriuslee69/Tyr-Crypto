...
proc checkedProduct(a, b: int, label: string): int =
  ...
  ## b: right factor.
  ## label: error label.
  if a < 0 or b <= 0 or a > high(int) div b:
...
proc validateParams(p: CustomKdfParams) =
  ...
  var
    ...
    blockCount: int = 0
  if p.roundCount < 1:
    raise newException(ValueError, "KDF round count must be positive")
  if p.memorySize <= 0:
  ...
  if uint64(p.hashCount) > uint64.high div uint64(p.roundCount):
    raise newException(ValueError, "KDF round/hash count product is too large")
...
proc usableMemorySize*(memorySize, blockSize: int): int =
  ...
  if memorySize < 0:
    raise newException(ValueError, "memory size must not be negative")
...
proc calcBlockIndexLeftBound*(blockIndex, blockSize: int): int =
  ...
  ## blockSize: logical block size.
  if blockIndex < 0 or blockSize <= 0:
    raise newException(ValueError, "KDF block index and size must be valid")
  result = checkedProduct(blockIndex, blockSize, "KDF block bound")
...
proc calcBlockIndexRightBound*(blockIndex, blockSize: int): int =
  ...
  result = calcBlockIndexLeftBound(blockIndex, blockSize)
  if result > high(int) - blockSize:
    raise newException(ValueError, "KDF block bound is too large")
...
proc xorBlockIntoMemory(M: var seq[uint8], blockIndex: int,
    B: openArray[uint8], blockSize: int) =
  ## M: flat KDF memory byte array.
...
proc xorTailBlocksIntoMemory(M: var seq[uint8], blockCount, blockSize: int) =
  ...
  while tailOffset < customKdfTailBlockCount:
    ...
    xorBlockIntoMemory(M, targetIndex, sourceBlock, blockSize)
...
proc deriveCustomKdf*(seed: openArray[uint8], p: CustomKdfParams,
  ...
  defer:
    secureClearBytes(M)
  if g == nil:
    raise newException(ValueError, "KDF requires a block generator")
  validateParams(p)
