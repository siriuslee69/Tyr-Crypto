...
proc blake3Compress*(cv: array[8, uint32], words: array[16, uint32], counter: uint64, blkLen, flags: uint32): array[16, uint32] =
  ...
  ## blkLen: block length.
  ## flags: BLAKE3 flags.
  if blkLen > uint32(blockLen):
    raise newException(ValueError, "BLAKE3 block length exceeds 64 bytes")
...
proc blake3CompressBatch*(
  ...
  if cvs.len != blocks.len:
    raise newException(ValueError, "BLAKE3 batch inputs must have equal lengths")
  if blkLen > uint32(blockLen):
    raise newException(ValueError, "BLAKE3 block length exceeds 64 bytes")
...
proc chunkOutput(chunk: openArray[byte], chunkIndex: uint64, key: array[8, uint32], baseFlags: uint32): Output =
  ...
  defer:
    secureClearPod(cv)
    secureClearPod(blockWords)
    secureClearPod(blockCv)
...
proc updateBlake3*(s: var Blake3Hasher, bs: openArray[byte]) =
  ...
  if not s.initialized:
    raise newException(ValueError, "BLAKE3 hasher is not initialized")
  i = 0
  while i < bs.len:
...
proc clearBlake3*(s: var Blake3Hasher) {.raises: [].} =
  ...
  var
    i: int = 0
  secureClearPod(s.key)
  secureClearBytes(s.chunkBuf)
  while i < s.stackCvs.len:
    secureClearPod(s.stackCvs[i])
    i = i + 1
  secureClearPod(s.lastOutput)
  s.stackCvs.setLen(0)
  s.stackLevels.setLen(0)
  s.chunkBuf.setLen(0)
  s.baseFlags = 0'u32
  s.chunkIndex = 0'u64
  s.chunkBufLen = 0
