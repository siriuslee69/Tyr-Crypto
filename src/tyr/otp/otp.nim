...
import ../ciphers/chacha/xchacha20_simd
...
proc sha1Digest(input: openArray[byte]): array[20, byte] =
  var
    ...
    i = 0
  defer:
    secureClearBytes(padded)
  if uint64(input.len) > uint64.high div 8'u64 or input.len > high(int) - 128:
    raise newException(ValueError, "SHA-1 input length is too large")
...
proc hmacSha1(key, msg: openArray[byte]): array[20, byte] =
  var
    ...
    keyHash: array[20, byte]
    i = 0
  defer:
    secureClearBytes(keyBlock)
    secureClearBytes(keyMaterial)
    secureClearBytes(inner)
    secureClearBytes(outer)
    secureClearBytes(digest)
    secureClearBytes(keyHash)
  if msg.len > high(int) - 64:
    raise newException(ValueError, "HMAC-SHA1 message is too large")
  keyBlock = newSeq[byte](64)
  inner = newSeq[byte](64 + msg.len)
  outer = newSeq[byte](64 + 20)
  if key.len > 64:
    keyHash = sha1Digest(key)
    keyMaterial = newSeq[byte](20)
    while i < 20:
      keyMaterial[i] = keyHash[i]
      i.inc
  else:
...
proc requireOtpInputs(secret: openArray[byte], digits: int) =
  ## Reject weak values instead of silently clamping them.
  if secret.len < 16:
    raise newException(ValueError, "OTP secret must be at least 16 bytes")
  if digits < 6 or digits > 8:
    raise newException(ValueError, "OTP digits must be in 6..8")

proc elapsedSeconds(unixTime, t0: int64): uint64 =
  if unixTime < t0:
    raise newException(ValueError, "unixTime must be >= t0")
  if t0 >= 0'i64 or unixTime < 0'i64:
    return uint64(unixTime - t0)
  result = uint64(unixTime) + uint64(-(t0 + 1'i64)) + 1'u64
...
proc hotp*(algo: OtpAlgo; secret: openArray[byte]; counter: uint64;
  ...
  requireOtpInputs(secret, digits)
  result = otpFromDigest(otpDigest(algo, secret, counter, b), digits)
...
proc hotpRfc4226*(secret: openArray[byte]; counter: uint64;
  ...
  result = otpFromDigest(mac, digits)
...
proc hotpBatch*(algo: OtpAlgo; secret: openArray[byte];
  ...
  while i < digests.len:
    result[i] = otpFromDigest(digests[i], digits)
    i.inc
...
proc totp*(algo: OtpAlgo; secret: openArray[byte]; unixTime: int64;
  ...
  var ctr = elapsedSeconds(unixTime, t0) div uint64(stepSec)
