...
import ../ciphers/chacha20 as chachaCore
...
when defined(tyrWasm):
  ...
  EM_JS(void, tyr_emscripten_random, (unsigned char *buffer, size_t size), {
    ...
    for (let offset = 0; offset < size; offset += maxRequest) {
      const take = Math.min(maxRequest, size - offset);
      provider.getRandomValues(HEAPU8.subarray(buffer + offset,
        buffer + offset + take));
    }
  });
  """.}
...
proc cryptoRandomBytesInternal(length: int, extraEntropy: openArray[uint8]): seq[uint8] =
  var
    ...
    i: int = 0
  defer:
    secureClearBytes(osSeed)
    secureClearBytes(osOutput)
    secureClearBytes(keyState)
    secureClearBytes(nonceBase)
    secureClearBytes(nonceDerive)
    secureClearBytes(mixMaterial)
    secureClearBytes(nonceTail)
    secureClearBytes(streamNonce)
    secureClearBytes(stream)
  if length < 0:
    raise newException(ValueError, "length must be >= 0")
  chachaCore.requireChaCha20BlockRange(0'u32, length)
  if length == 0:
    return @[]
