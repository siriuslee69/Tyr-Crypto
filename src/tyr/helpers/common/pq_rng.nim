...
proc nistDrbgRandomBytes*(S: var PqNistDrbgState, n: int): seq[byte] =
  ...
  var
    ...
    i: int = 0
  if n < 0:
    raise newException(ValueError, "NIST DRBG byte count must be non-negative")
  if n > 65_536:
    raise newException(ValueError, "NIST DRBG request exceeds 65,536 bytes")
  if S.reseedCounter <= 0:
    raise newException(ValueError, "NIST DRBG state is not initialized")
  if uint64(S.reseedCounter) > (1'u64 shl 48):
    raise newException(ValueError, "NIST DRBG reseed interval exceeded")
