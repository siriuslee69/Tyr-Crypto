...
proc functionK(m: BikeMessage, ctBytes: openArray[byte]): BikeSharedSecret =
  var
    tmp: seq[byte] = @[]
    dgst: seq[byte] = @[]
    i: int = 0
  ...
  tmp = newSeq[byte](bikeMessageBytes + bikeCiphertextBytes)
  copyMem(addr tmp[0], unsafeAddr m[0], bikeMessageBytes)
  copyMem(addr tmp[bikeMessageBytes], unsafeAddr ctBytes[0], bikeCiphertextBytes)
  dgst = sha3_384(tmp)
  i = 0
...
proc bikeTyrEncapsDerand*(v: BikeVariant, pk: openArray[byte],
  ...
  ss = functionK(m, result.ciphertext)
...
proc bikeTyrTryDecapsInternal(v: BikeVariant, sk,
    ctBytes: openArray[byte]): tuple[sharedSecret: seq[byte], ok: bool] =
  ...
  canonicalCt = uint32(ord(rawPolyPaddingIsCanonical(ctBytes)))
  ...
  successCond = successCond and canonicalCt
...
proc bikeTyrDecaps*(v: BikeVariant, sk, ctBytes: openArray[byte]): seq[byte] {.otterTrace.} =
  ...
  result = bikeTyrTryDecapsInternal(v, sk, ctBytes).sharedSecret

when defined(tyrCryptoTestHooks):
  ## Reference: [BIKE-5.2] section 4, decapsulation algorithm; test-only diagnostic exposure must remain compile-time gated and must not alter implicit rejection.
  proc bikeTyrTryDecaps*(v: BikeVariant, sk,
      ctBytes: openArray[byte]): tuple[sharedSecret: seq[byte], ok: bool] =
    ## Test-only visibility for implicit-rejection regression checks.
    result = bikeTyrTryDecapsInternal(v, sk, ctBytes)
