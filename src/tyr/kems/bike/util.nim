...
## Reference: [BIKE-5.2] sections 2-4, BIKE KEM and BGF decoder algorithms; canonical byte and polynomial encoding rules for `rawPolyPaddingIsCanonical`; pitfall: reject nonzero padding bits instead of silently aliasing another encoding.
proc rawPolyPaddingIsCanonical*(A: openArray[byte], o: int = 0): bool =
  if o < 0 or A.len < bikeRBytes or o > A.len - bikeRBytes:
    return false
  result = (A[o + bikeRBytes - 1] and (not bikeLastRByteMask)) == 0'u8

## Reference: [BIKE-5.2] sections 2-4, BIKE KEM and BGF decoder algorithms; canonical byte and polynomial encoding rules for `requireCanonicalRawPoly`; pitfall: reject nonzero padding bits instead of silently aliasing another encoding.
proc requireCanonicalRawPoly(A: openArray[byte], o: int, label: string) =
  if not rawPolyPaddingIsCanonical(A, o):
    raise newException(ValueError, label & " has nonzero padding bits")
...
proc parsePublicKey*(A: openArray[byte]): BikeRawPoly =
  ...
  requireCanonicalRawPoly(A, 0, "BIKE public key")
  i = 0
  while i < bikeRBytes:
    result[i] = A[i]
    i = i + 1
...
proc validateSecretIndexList(W: BikeIndexList, R: BikeRawPoly) =
  var
    ...
    pos: int = 0
  defer:
    secureZeroPod(expected)
  while i < bikeD:
    if W[i] >= uint32(bikeRBits):
      raise newException(ValueError, "BIKE secret index is out of range")
    pos = int(W[i])
    if getBitRaw(expected, pos) != 0'u8:
      raise newException(ValueError, "BIKE secret index list contains duplicates")
    setBitRaw(expected, pos)
    i = i + 1
  if secureCmpBytes(expected, R) != 1'u32:
    raise newException(ValueError, "BIKE secret indices do not match polynomial")
...
proc parseSecretKey*(A: openArray[byte]): BikeSecretKeyState =
  ...
  i = 0
  while i < bikeN0:
    requireCanonicalRawPoly(A, o, "BIKE secret polynomial")
    copyMem(addr result.bin[i][0], unsafeAddr A[o], bikeRBytes)
    o = o + bikeRBytes
    i = i + 1
  requireCanonicalRawPoly(A, o, "BIKE secret public key")
  copyMem(addr result.pk[0], unsafeAddr A[o], bikeRBytes)
  o = o + bikeRBytes
  copyMem(addr result.sigma[0], unsafeAddr A[o], bikeMessageBytes)
  validateSecretIndexList(result.wlist[0], result.bin[0])
  validateSecretIndexList(result.wlist[1], result.bin[1])
