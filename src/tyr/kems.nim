...
proc keypair*(v: KyberVariant, seed: seq[byte] = @[]): KemKeypair {.role: {orchestrator}.} =
...
proc decaps*(v: McElieceVariant, sk, ct: openArray[byte]): seq[byte] {.role: {decryptor}.} =
  ## v/sk/ct: variant, your secret key, the ciphertext you received.
  result = mcelieceTyrDecaps(v, sk, ct)

proc decaps*(v: FrodoVariant, sk, ct: openArray[byte]): seq[byte] {.role: {decryptor}.} =
  ## v/sk/ct: variant, your secret key, the ciphertext you received.
  result = frodoTyrDecaps(v, sk, ct)

proc decaps*(v: BikeVariant, sk, ct: openArray[byte]): seq[byte] {.role: {decryptor}.} =
  ## v/sk/ct: variant, your secret key, the ciphertext you received.
  result = bikeTyrDecaps(v, sk, ct)

proc decaps*(v: NtruVariant, sk, ct: openArray[byte]): seq[byte] {.role: {decryptor}.} =
  ## v/sk/ct: variant, your secret key, the ciphertext you received.
