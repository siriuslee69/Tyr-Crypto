...
proc requireHmacTagLen(outLen: int) {.role: {parser}.} =
  ## outLen: high-level authentication tag length in bytes.
  if outLen < hmacMinTagLen:
    raise newException(ValueError, "hmac authentication tags must be at least 16 bytes")
...
proc normalizeHmacKeyWithHash*(key: openArray[byte], blockLen, outLen: int,
  ...
  if key.len > blockLen:
    if hashFn == nil:
      raise newException(ValueError, "hmac key reduction requires a hash function")
    hashLen = keyHashLen
    if hashLen <= 0:
      hashLen = resolvedHmacKeyHashLen(blockLen, outLen)
    if hashLen > blockLen:
      raise newException(ValueError,
        "hmac key reduction output cannot exceed the block length")
    keyMaterial = hashFn(key, hashLen)
    if keyMaterial.len != hashLen:
      raise newException(ValueError, "hmac key reducer returned wrong length")
  else:
...
proc prepareKeyedHmacKey*(keyMaterial: openArray[byte], keyedKeyLen: int,
  ...
  if reduced.len != keyedKeyLen:
    raise newException(ValueError, "keyed hmac reducer returned wrong length")
...
proc customHmacFromHash*(key, msg: openArray[byte], blockLen, outLen: int,
  ...
  if hashFn == nil:
    raise newException(ValueError, "hmac requires a hash function")
  ...
  if mac.len != outLen:
    raise newException(ValueError, "hmac hash function returned wrong length")
  outerInput = @[]
...
proc customHmacFromKeyedHash*(key, msg: openArray[byte], blockLen, outLen: int,
  ...
  defer:
    ...
    secureClearBytes(mac)
  if keyedHashFn == nil:
    raise newException(ValueError, "keyed hmac requires a keyed hash function")
...
proc hmacVerify*(expected, actual: openArray[byte]): bool =
  ...
  if expected.len < hmacMinTagLen or actual.len < hmacMinTagLen:
    return false
