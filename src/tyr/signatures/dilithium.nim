...
template sampleIntermediateVectorGamma1(p, z, seed, nonceBase: untyped) =
  if nonceBase > uint16.high - uint16(p.l):
    raise newException(ValueError, "Dilithium signing nonce space exhausted")
...
proc dilithiumTyrSignDerandInto*(v: DilithiumVariant, sig: var openArray[byte], msg: openArray[byte],
  ...
  if polyveclChkNorm(skp.s1, int32(p.eta + 1)) or
      polyveckChkNorm(skp.s2, int32(p.eta + 1)):
    raise newException(ValueError, "Dilithium secret key has invalid eta encoding")
