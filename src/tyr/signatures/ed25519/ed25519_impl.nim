...
proc ed25519Sha512Hash*(A: openArray[byte]): Ed25519Bytes64 =
  ...
  var
    ...
    w: array[80, uint64] = default(array[80, uint64])
    a, b, c, d, e, f, g, hh: uint64 = 0'u64
    s0, s1, ch, maj, temp1, temp2: uint64 = 0'u64
    offset: int = 0
    i: int = 0
  if uint64(A.len) > uint64.high div 8'u64 or A.len > high(int) - 128:
    raise newException(ValueError, "SHA-512 input length is too large")
  defer:
...
proc feNeg(h: var Ed25519Field, f: Ed25519Field) {.inline.} =
  var z: Ed25519Field = default(Ed25519Field)
  fe0(z)
  feSub(h, z, f)
...
proc feMul(h: var Ed25519Field, f, g: Ed25519Field) {.inline.} =
  var
    ...
    f3_19 = 19'u64 * f[3]
    f4_19 = 19'u64 * f[4]
    r0lo, r0hi, r1lo, r1hi, r2lo, r2hi, r3lo, r3hi, r4lo, r4hi: uint64 = 0'u64
...
proc feToBytes(h: Ed25519Field): Ed25519Bytes32 {.inline.} =
  var t: Ed25519Field = default(Ed25519Field)
  feReduce(t, h)
...
proc fePow(h: var Ed25519Field, z: Ed25519Field, e: Ed25519Bytes32) =
  var
    r: Ed25519Field = default(Ed25519Field)
    i: int = 255
  fe1(r)
  while i >= 0:
    feSq(r, r)
...
proc pointAdd(r: var Ed25519Point, p, q: Ed25519Point) =
  var a, b, c, d, e, f, g, h, t0, t1: Ed25519Field = default(Ed25519Field)
  defer:
    secureClearPod(a)
    secureClearPod(b)
    secureClearPod(c)
...
proc pointDouble(r: var Ed25519Point, p: Ed25519Point) =
  var a, b, c, e, f, g, h, t0: Ed25519Field = default(Ed25519Field)
  defer:
    secureClearPod(a)
    secureClearPod(b)
    secureClearPod(c)
...
proc pointEncode(p: Ed25519Point): Ed25519Bytes32 =
  var
    zi, x, y: Ed25519Field = default(Ed25519Field)
  feInvert(zi, p.z)
  feMul(x, p.x, zi)
  feMul(y, p.y, zi)
  result = feToBytes(y)
...
proc pointDecode(p: var Ed25519Point, s: Ed25519Bytes32): bool =
  var
    ...
    y, y2, u, v, vInv, x2, x, check: Ed25519Field = default(Ed25519Field)
    one: Ed25519Field = default(Ed25519Field)
