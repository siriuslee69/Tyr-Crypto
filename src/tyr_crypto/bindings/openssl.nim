...
when defined(hasOpenSSL3):
  ...
  proc promptBuildAndReload(): bool =
    ...
    var sourceDir = getEnv("OPENSSL_SOURCE").strip()
    if sourceDir.len == 0:
      sourceDir = joinPath(getCurrentDir(), "..", "openssl")
    if promptAndBuildOpenSsl(extraLibCandidates, sourceDir):
      return true
    false
