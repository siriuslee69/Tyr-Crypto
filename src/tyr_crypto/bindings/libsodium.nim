...
when defined(hasLibsodium):
  ...
  proc defaultLibCandidates(): seq[string] =
    ...
    appendLibCandidates(candidates, joinPath(moduleSource, "build", "install", "lib"))
    appendLibCandidates(candidates, joinPath(moduleSource, "build", "lib"))
    appendLibCandidates(candidates, joinPath(moduleSource, "build"))
    appendLibCandidates(candidates, joinPath(moduleSource, "zig-out", "lib"))
    appendLibCandidates(candidates, joinPath(moduleSource, "zig-out", "bin"))
    candidates
  ...
  proc attemptBuildAndReload(): bool =
    ...
    var sourceDir = getEnv("LIBSODIUM_SOURCE").strip()
    if sourceDir.len == 0:
      sourceDir = joinPath(moduleDir, "..", "..", "libsodium")
    autoBuildLibsodium(extraLibCandidates, sourceDir)
