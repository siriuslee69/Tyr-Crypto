...
proc resolveTagLen(opt: ChunkyOptions, s: EncryptionState): uint16 =
  if opt.tagLen != 0'u16:
    return opt.tagLen
  if s.tagLen != 0'u16:
    return s.tagLen
...
proc requireKey32(s: EncryptionState, idx: int, name: string): array[32, uint8] =
  var
    rs: array[32, uint8]
    i: int = 0
  if s.keys.len <= idx:
    raise newException(ValueError, "missing symmetric key for " & name)
  if s.keys[idx].key.len != 32:
    raise newException(ValueError, "invalid " & name & " key length")
  i = 0
  while i < rs.len:
    rs[i] = s.keys[idx].key[i]
    i = i + 1
  result = rs

proc requireNonce24(s: EncryptionState): array[24, uint8] =
  var
    rs: array[24, uint8]
    i: int = 0
  if s.nonce.len != 24:
...
proc encryptChunkTask(t: var ChunkEncryptTask) =
  ...
  try:
    ...
    initChunkCryptoState(st, t.keyXs, t.keyAs, t.keyGs, ns, t.bufferBytes)
...
proc decryptChunkTask(t: var ChunkDecryptTask) =
  ...
  try:
    ...
    initChunkCryptoState(st, t.keyXs, t.keyAs, t.keyGs, h.nonce, t.bufferBytes)
    buf.setLen(t.bufferBytes)
...
proc encryptFileChunks*(i: string, o: string, s: EncryptionState,
  ...
  if opt.algo != caXChaCha20AesGimli:
    raise newException(ValueError, "unsupported chunk algorithm")
  if s.algoType != xchacha20AesGimli:
...
proc decryptFileChunks*(m: ChunkyManifest, iDir: string, oFile: string,
    s: EncryptionState, opt: ChunkyOptions) =
  ## m: chunk manifest.
