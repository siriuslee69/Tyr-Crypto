...
suite "dilithium tyr":
  test "poly add sub and shift match scalar reference":
    var
      a: dilithium_poly.DilithiumPoly
      b: dilithium_poly.DilithiumPoly
      addRes: dilithium_poly.DilithiumPoly
      subRes: dilithium_poly.DilithiumPoly
      shiftRes: dilithium_poly.DilithiumPoly
      i: int = 0
    i = 0
    while i < custom_dilithium.dilithiumN:
      ...
      i = i + 1
    dilithium_poly.polyAdd(addRes, a, b)
    dilithium_poly.polySub(subRes, a, b)
    dilithium_poly.polyShiftL(shiftRes)
    i = 0
    while i < custom_dilithium.dilithiumN:
  ...
  test "ML-DSA verification rejects malformed signature and public-key boundaries":
    var
      v = custom_dilithium.dilithium44
      p = custom_dilithium.dilithiumParamsTable[v]
      seed = newSeq[byte](32)
      msg = @[0x62'u8, 0x6f'u8, 0x75'u8, 0x6e'u8, 0x64'u8]
      kp: custom_dilithium.DilithiumTyrKeypair
      sig, malformedSig, malformedPk: seq[byte]
      i: int = 0
    fillDiliSeed(seed, 31)
    kp = custom_dilithium.dilithiumTyrKeypair(v, seed)
    sig = custom_dilithium.dilithiumTyrSignDeterministic(v, msg,
      kp.secretKey)
    check custom_dilithium.dilithiumTyrVerify(v, msg, sig, kp.publicKey)

    malformedSig = sig
    malformedSig.setLen(p.signatureBytes - 1)
    check not custom_dilithium.dilithiumTyrVerify(v, msg, malformedSig,
      kp.publicKey)
    malformedSig = sig
    ...
    check not custom_dilithium.dilithiumTyrVerify(v, msg, sig, malformedPk)
    malformedPk = kp.publicKey
    malformedPk.add(0'u8)
    check not custom_dilithium.dilithiumTyrVerify(v, msg, sig, malformedPk)

    malformedSig = newSeq[byte](p.signatureBytes)
  ...
  test "ML-DSA signing rejects invalid eta encodings in secret keys":
    var
      seed = newSeq[byte](32)
      msg = @[byte 0x65, 0x74, 0x61]
      etaOffset: int = 2 * custom_dilithium.dilithiumSeedBytes +
        custom_dilithium.dilithiumTrBytes
      kp: custom_dilithium.DilithiumTyrKeypair
      malformedSk: seq[byte] = @[]
    fillDiliSeed(seed, 101)
    for v in [custom_dilithium.dilithium44, custom_dilithium.dilithium65,
        custom_dilithium.dilithium87]:
      kp = custom_dilithium.dilithiumTyrKeypair(v, seed)
      malformedSk = kp.secretKey
      malformedSk[etaOffset] = 0xff'u8
      expect ValueError:
        discard custom_dilithium.dilithiumTyrSignDeterministic(v, msg,
          malformedSk)
