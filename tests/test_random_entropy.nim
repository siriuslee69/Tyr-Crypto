...
suite "entropy-mixed crypto random":
  ...
  test "zero length does not request or allocate entropy":
    check cryptoRandomBytes(0).len == 0
  ...
  when sizeof(int) >= 8:
    test "rejects lengths beyond the XChaCha20 counter range before allocation":
      expect ValueError:
        discard cryptoRandomBytes(274_877_906_945)
