...
suite "kyber tyr":
  test "derandomized keypair validates material before slicing":
    expect ValueError:
      discard custom_kyber.kyberTyrKeypairDerand(custom_kyber.kyber768,
        newSeq[byte](63))
    expect ValueError:
      discard custom_kyber.kyberTyrKeypairDerand(custom_kyber.kyber768,
        newSeq[byte](65))
  ...
  test "invalid Kyber ciphertext uses deterministic implicit rejection":
    var
      ...
      allZero: seq[byte] = @[]
      allFf: seq[byte] = @[]
      fallbackMaterial: array[2 * kyberSymBytes, byte]
    ...
    kp = custom_kyber.kyberTyrKeypair(custom_kyber.kyber768, keySeed)
    env = custom_kyber.kyberTyrEncaps(custom_kyber.kyber768,
      kp.publicKey, encapsSeed)
    expect ValueError:
      discard custom_kyber.kyberTyrDecaps(custom_kyber.kyber768,
        kp.secretKey, @[])
    expect ValueError:
      discard custom_kyber.kyberTyrDecaps(custom_kyber.kyber768,
        kp.secretKey, newSeq[byte](env.ciphertext.len - 1))
    expect ValueError:
      discard custom_kyber.kyberTyrDecaps(custom_kyber.kyber768,
        kp.secretKey, newSeq[byte](env.ciphertext.len + 1))
    tampered = env.ciphertext
    tampered[0] = tampered[0] xor 1'u8
    bad0 = custom_kyber.kyberTyrDecaps(custom_kyber.kyber768,
      kp.secretKey, tampered)
    bad1 = custom_kyber.kyberTyrDecaps(custom_kyber.kyber768,
      kp.secretKey, tampered)
    ...
    while i < allFf.len:
      allFf[i] = 0xff'u8
      i = i + 1
    hashHInto(ctHash, allZero)
    i = 0
    while i < kyberSymBytes:
      fallbackMaterial[i] = kp.secretKey[p.secretKeyBytes - kyberSymBytes + i]
      fallbackMaterial[kyberSymBytes + i] = ctHash[i]
      i = i + 1
    expectedZero = newSeq[byte](kyberSharedSecretBytes)
    kdfInto(expectedZero, fallbackMaterial)
    zero0 = custom_kyber.kyberTyrDecaps(custom_kyber.kyber768,
      kp.secretKey, allZero)
    zero1 = custom_kyber.kyberTyrDecaps(custom_kyber.kyber768,
      kp.secretKey, allZero)
    ff0 = custom_kyber.kyberTyrDecaps(custom_kyber.kyber768,
      kp.secretKey, allFf)
    ff1 = custom_kyber.kyberTyrDecaps(custom_kyber.kyber768,
      kp.secretKey, allFf)
    check zero0 == zero1
