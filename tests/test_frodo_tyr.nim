...
suite "frodo tyr":
  test "constant-time word verification returns full-byte masks":
    var
      ...
      selector: int8 = 0
    check frodo_util.ctVerifyWords(equalA, equalB) == 0'i8
    selector = frodo_util.ctVerifyWords(equalA, changed)
    check selector == -1'i8
    frodo_util.ctSelectBytes(selected, accepted, rejected, selector)
    check selected == rejected

  test "pure-nim Frodo1344Aes roundtrip matches shared secret":
    runPureRoundtripCase(custom_frodo.frodo1344aes, 29, 93)

  test "malformed Frodo ciphertexts reject or derive deterministic fallback secrets":
    var
      v = custom_frodo.frodo640aes
      p = custom_frodo.frodoParamsTable[v]
      keypairRandom = newSeq[byte](p.keypairRandomBytes)
      encapsRandom = newSeq[byte](p.encapsRandomBytes)
      ...
      ff1: seq[byte] = @[]
      i: int = 0
    fillSeed(keypairRandom, 47)
    fillSeed(encapsRandom, 109)
    kp = custom_frodo.frodoTyrKeypairDerand(v, keypairRandom)
    env = custom_frodo.frodoTyrEncapsDerand(v, kp.publicKey, encapsRandom)
    expect ValueError:
      discard custom_frodo.frodoTyrDecaps(v, kp.secretKey, @[])
    expect ValueError:
      discard custom_frodo.frodoTyrDecaps(v, kp.secretKey,
        newSeq[byte](p.ciphertextBytes - 1))
    expect ValueError:
      discard custom_frodo.frodoTyrDecaps(v, kp.secretKey,
        newSeq[byte](p.ciphertextBytes + 1))
    ...
    while i < allFf.len:
      allFf[i] = 0xff'u8
      i = i + 1
    fallbackInput = newSeq[byte](p.ciphertextBytes + p.sharedSecretBytes)
    i = 0
    while i < p.ciphertextBytes:
      fallbackInput[i] = allZero[i]
      i = i + 1
    i = 0
    while i < p.sharedSecretBytes:
      fallbackInput[p.ciphertextBytes + i] = kp.secretKey[i]
      i = i + 1
    expectedZero = newSeq[byte](p.sharedSecretBytes)
    case p.xofKind
    of custom_frodo.fxShake128:
      tyr_sha3.shake128Into(expectedZero, fallbackInput)
    of custom_frodo.fxShake256:
      tyr_sha3.shake256Into(expectedZero, fallbackInput)
    zero0 = custom_frodo.frodoTyrDecaps(v, kp.secretKey, allZero)
    zero1 = custom_frodo.frodoTyrDecaps(v, kp.secretKey, allZero)
    ff0 = custom_frodo.frodoTyrDecaps(v, kp.secretKey, allFf)
    ff1 = custom_frodo.frodoTyrDecaps(v, kp.secretKey, allFf)
    check zero0 == zero1
  ...
  when not defined(release):
    test "full pure-nim Frodo variant matrix is release-only due runtime cost":
      checkpoint("The remaining Frodo pure-Nim runtime checks require a release build")
  else:
