...
suite "custom crypto":
  ...
  test "Custom KDF rejects unusable parameters":
    var
      p = initCustomKdfParams(1, 512, 1, 8)
      nilGenerator: CustomKdfBlockGenerator
    expect ValueError:
    ...
    expect ValueError:
      discard deriveCustomKdf(@[byte 1], ckaGimli, 0, 512, 1, 8)
    expect ValueError:
      discard deriveCustomKdf(@[byte 1], p, nilGenerator)

  test "BLAKE3 Gimli KDF frames salt and secret boundaries":
    let
      a = deriveBlake3GimliBlocks("framing", @[byte 2, 3], @[byte 1], 1, 1)
      b = deriveBlake3GimliBlocks("framing", @[byte 3], @[byte 1, 2], 1, 1)
    check a != b

  test "BLAKE3 Gimli KDF rejects invalid config and overflowing ranges":
    expect ValueError:
      discard deriveBlake3GimliBlocks("range", @[byte 1], @[byte 2],
        high(int), 2)
    expect ValueError:
      discard deriveBlake3GimliBlocks("config", @[byte 1], @[byte 2], 1, 1,
        cfg = Blake3GimliKdfConfig(keyBytes: 32, nonceBytes: 8))
    expect ValueError:
      discard deriveBlake3GimliNonce(@[byte 1],
        cfg = Blake3GimliKdfConfig(keyBytes: 32, nonceBytes: 8))

  test "BLAKE3 Gimli KDF supports non-default output block sizes":
    var
      cfg = initBlake3GimliKdfConfig(keyBytes = 48)
      blocks = deriveBlake3GimliBlocks("wide", @[byte 1], @[byte 2], 1, 2,
        cfg = cfg)
      stage = deriveBlake3GimliStageKey(@[byte 1], @[byte 2], 1, cfg = cfg)
    check blocks.len == 96
    check stage.len == 48
