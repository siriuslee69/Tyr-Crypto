...
suite "custom x25519":
  ...
  test "raw rejection clears caller-owned output":
    var
      outShared: X25519Bytes32
      secretKey = toFixed32(hexToBytes(curve25519Vector.skHex))
      peer = smallOrderBlocklist[0]
      i: int = 0
    while i < outShared.len:
      outShared[i] = 0xa5'u8
      i = i + 1
    check not rawX25519.x25519ScalarmultRaw(outShared, secretKey, peer)
    check outShared == default(X25519Bytes32)
