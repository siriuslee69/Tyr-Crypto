...
when defined(hasLibOqs):
  proc oqsDiliKatCallback(random_array: ptr uint8,
    ...
    if not oqsDiliKatDrbgReady:
      ...
      return
    bytesBuf = diliDrbgRandomBytes(oqsDiliKatDrbgState, int(bytes_to_read))
...
proc sha256HexForDiliFile(path: string): string =
  when defined(windows):
    var
      escaped: string = path.replace("'", "''")
      output: string = execProcess(
