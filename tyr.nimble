...
task test, "Run the crypto bindings test suite":
  exec withRepoCaches("nim r --nimcache:" & repoNimcacheDir("nimcache_run_desktop_tests_parallel").replace('\\', '/') & " tests/run_desktop_tests_parallel.nim")

task test_all, "Run the full crypto bindings test suite with libsodium, liboqs, and OpenSSL":
  exec withRepoCaches("nim r --nimcache:" & repoNimcacheDir("nimcache_run_desktop_tests_parallel").replace('\\', '/') & " tests/run_desktop_tests_parallel.nim -- --full")

task test_all_threads_on, "Run test_all with threads enabled":
  exec withRepoCaches("nim r --nimcache:" & repoNimcacheDir("nimcache_run_desktop_tests_parallel").replace('\\', '/') & " tests/run_desktop_tests_parallel.nim -- --full --childNimFlags:\"--gc:orc --threads:on\"")

task test_all_threads_off, "Run test_all with threads disabled":
  exec withRepoCaches("nim r --nimcache:" & repoNimcacheDir("nimcache_run_desktop_tests_parallel").replace('\\', '/') & " tests/run_desktop_tests_parallel.nim -- --full --childNimFlags:\"--gc:orc --threads:off\"")
...
task test_nugimli, "Run NuGimli reversible block permutation tests":
  exec withRepoCaches("nim c --nimcache:" & repoNimcacheDir("nimcache_test_nugimli").replace('\\', '/') & " -r tests/test_nugimli.nim")
...
task evaluate_nugimli_streams, "Run full threaded NuGimli stream campaign":
  exec withRepoCaches("nim c -r --threads:on -d:release --path:" & otterSrcDir() &
    ...
    " tests/evaluate_nugimli_streams.nim -- --quiet")
...
task evaluate_nugimli_streams_quick, "Run quick threaded NuGimli stream campaign":
  exec withRepoCaches("nim c -r --threads:on -d:release --path:" & otterSrcDir() &
    ...
    " tests/evaluate_nugimli_streams.nim -- --quick --quiet")
...
task test_nugimli_streams, "Run threaded NuGimli stream campaign tests":
  exec withRepoCaches("nim c -r --threads:on --path:" & otterSrcDir() &
    ...
    " tests/test_nugimli_stream_campaign.nim")
...
task evaluate_nugimli_advanced, "Run Cascade algebraic and structural searches":
  exec withRepoCaches("nim c -r --threads:on -d:release --path:" & otterSrcDir() &
    ...
    " tests/evaluate_nugimli_advanced.nim")
...
task evaluate_nugimli_leakage, "Run Cascade fixed-random timing leakage tests":
  exec withRepoCaches("nim c -r -d:release --path:" & otterSrcDir() &
    ...
    " tests/evaluate_nugimli_leakage.nim")
...
task bench_pq_profiles, "Build matched scalar/AVX2 liboqs profiles and run Otter PQ comparison benches":
  exec withRepoCaches("nim r --nimcache:" & repoNimcacheDir("nimcache_bench_pq_profiles").replace('\\', '/') & " tests/bench_pq_profiles.nim")
...
task bench_custom_kdf, "Run the custom KDF generator benchmark table":
  exec withRepoCaches("nim c --threads:on --nimcache:" & repoNimcacheDir("nimcache_bench_custom_kdf").replace('\\', '/') & " -d:release -d:sse2 -d:avx2 -d:aesni --passC:\"-msse4.1 -mavx2 -maes\" --passL:\"-msse4.1 -mavx2\" -r tests/bench_custom_kdf.nim")
...
task build_libsodium, "Build libsodium and prepare combined headers":
  exec "nim r tests/ensure_env.nim -- --submodules --builddirs"
  exec "nim r tools/build_libsodium.nim"
  exec "nim r tests/prepare_libsodium_header.nim"

task build_liboqs, "Build liboqs and prepare combined headers":
  exec "nim r tests/ensure_env.nim -- --submodules --builddirs"
  exec "nim r tools/build_liboqs.nim"
  exec "nim r tests/prepare_liboqs_header.nim"
